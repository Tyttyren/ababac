...
class MultiPlateGenerator:
    ...
    def generate_plate(self, enhance=False):
        ...
        i = Image.open(os.path.join(self.adr_plate_model, '{}_{}.PNG'.format(bg_color, height)))
        # img_plate_model = cv2.imread(os.path.join(self.adr_plate_model, '{}_{}.PNG'.format(bg_color, height)))
        img_plate_model = np.asarray(i)
    ...
    def generate_plate_special(self, plate_number, bg_color, is_double, enhance=False):
        ...
        :param bg_color: 背景颜色，black/black_shi（使领馆）/blue/green_car（新能源轿车）/green_truck（新能源卡车）/white/white_army（军队）/yellow/yellow_
...
if __name__ == '__main__':
    ...
    mkdir(args.save_adr)
    generator = MultiPlateGenerator('./images', './font_model')
    # plate, number_xy, gt_plate_number, bg_color, is_double = generator.generate_plate()
    plate = generator.generate_plate_special('川A516253', 'yellow', True)
    Image.fromarray(plate).show()
    ...
    # img, number_xy, gt_plate_number, bg_color, is_double = generator.generate_plate_special('川A516253', 'yellow', True)
    # cv2.imwrite(os.path.join(args.save_adr, '{}_{}_{}.jpg'.format(gt_plate_number, bg_color, is_double)), img)
