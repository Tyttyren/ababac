...
# def detect_one(model,path):

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--detect_model', nargs='+', type=str, default='/model/plate_detect.pt', help='model.pt path(s)')  #检测模型
    parser.add_argument('--rec_model', type=str, default='/model/plate_rec_color.pth', help='model.pt path(s)')#车牌识别+颜色识别模型
    parser.add_argument('--is_color',type=bool,default=False,help='plate color')     #是否识别颜色
    parser.add_argument('--image_path', type=str, default='/imgs/1.jpg', help='source') 
    ...
    print(opt)
    # os.chdir(r'D:\\学习的\\project\\LicensePlateRecognition-main\\yolov5')
    abs = os.path.abspath('./')
    opt.detect_model = abs+opt.detect_model
    # opt.rec_model = abs+opt.rec_model
    # opt.image_path = abs+opt.image_path
